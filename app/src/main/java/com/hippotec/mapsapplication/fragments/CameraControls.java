...
public class CameraControls extends LinearLayout {
    public static int ACTIVITY_REQUEST_CODE_APPROVE_IMAGE = 1;
    ...
    private boolean allowVideoCapture = false;
    ...
    public void imageCaptured(CameraKitImage image) {
        ...
        PhotoResultHolder.dispose();
        PhotoResultHolder.setImage(jpeg);
        PhotoResultHolder.setNativeCaptureSize(cameraView.getCaptureSize());
        PhotoResultHolder.setTimeToCallback(callbackTime - captureStartTime);

        Activity activity = (Activity)getContext();
        Intent intent = new Intent(getContext(), ImagePreviewActivity.class);
        if (activity.getIntent().hasExtra(ImagePreviewActivity.PARAM_KEY_IS_EVENT_CREATION)) {
            intent.putExtra(ImagePreviewActivity.PARAM_KEY_IS_EVENT_CREATION, activity.getIntent().getBooleanExtra(ImagePreviewActivity.PARAM_KEY_IS_EVENT_CREATION, false));
        }
        activity.startActivityForResult(intent, ACTIVITY_REQUEST_CODE_APPROVE_IMAGE);
    }
    ...
    public void videoCaptured(CameraKitVideo video) {
        ...
        if (videoFile != null) {
            PhotoResultHolder.dispose();
            PhotoResultHolder.setVideo(videoFile);
            PhotoResultHolder.setNativeCaptureSize(cameraView.getCaptureSize());
            Intent intent = new Intent(getContext(), ImagePreviewActivity.class);
